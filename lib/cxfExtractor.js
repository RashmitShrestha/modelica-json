 ...
 *     The sub-sequences to serialize into the same graph. Each is a block of
 *     its own, so that the CXF of a sequence needs no companion file to
 *     resolve the components it contains.
...
function getCxfGraph (instances, requiredReferences, blockName, generateElementary, generateCxfCore, subBlocks = []) {
  ...
  /* A sub-sequence carries the values of the instantiation it was resolved at,
     so two instantiations of the same Modelica class are two distinct blocks.
     They share the graph, and each keeps its own instance map, so that the
     names they have in common do not collide. */
