...
/**
 * adds every node of one block to a CXF graph
 *
 * The graph is mutated in place so that several blocks can share it: a
 * sub-sequence resolved at one instantiation is a block of its own, and it
 * belongs in the same file as the sequence that instantiates it.
 *
 * @param {Object} graph - The rdflib graph to add the nodes to.
 * @param {Object} instances - The instances of the block.
 * @param {Object} requiredReferences - The connections and extends clauses.
 * @param {string} blockName - The name the block takes in the graph.
 * @param {boolean} generateElementary - Whether to emit elementary blocks.
 * @param {boolean} generateCxfCore - Whether CXF-core is being generated.
 * @returns {boolean} - Whether the block was added.
 */
function addBlockToGraph (graph, instances, requiredReferences, blockName, generateElementary, generateCxfCore) {
  let instancesList = []
  ...
  instancesList.forEach(instance => {
    ...
    if (instanceType === 'long_class_specifier') {
      ...
      if ((isElementaryBlock && generateElementary && instanceAccessSpecifier === 'public') || (isElementaryBlock && generateCxfCore && instanceAccessSpecifier === 'public') || !isElementaryBlock) {
        ...
        // type specifier of the component is either the sub-sequence it instantiates or its declared type
        const typeSpecifier = instanceDict.subInstanceDefinition ?? instanceDict.type_specifier
...
/**
 * serializes one block, and every sub-sequence it resolved, into one CXF graph
 *
 * @param {Object} instances - The instances of the block.
 * @param {Object} requiredReferences - The connections and extends clauses.
 * @param {string} blockName - The name the block takes in the graph.
 * @param {boolean} generateElementary - Whether to emit elementary blocks.
 * @param {boolean} generateCxfCore - Whether CXF-core is being generated.
 * @param {Array<{blockName: string, instances: Object, requiredReferences: Object}>} [subBlocks]
 *     The sub-sequences to serialize into the same graph. At the objects json
 *     layer each of these is a file of its own, found through the `subInstanceDefinition`
 *     field of the component that instantiates it; here they are all folded
 *     into the one graph, so that the CXF of a sequence is standalone and needs
 *     no companion file to resolve the blocks it contains.
 * @returns {Object|null} - The CXF graph, or null when the block is not valid CDL.
 */
function getCxfGraph (instances, requiredReferences, blockName, generateElementary, generateCxfCore, subBlocks = []) {
  ...
  if (!addBlockToGraph(graph, instances, requiredReferences, blockName, generateElementary, generateCxfCore)) {
    return null
  }

  // add sub-sequences to the graph
  subBlocks.forEach(subBlock => {
    addBlockToGraph(graph, subBlock.instances, subBlock.requiredReferences,
      subBlock.blockName, generateElementary, generateCxfCore)
  })
