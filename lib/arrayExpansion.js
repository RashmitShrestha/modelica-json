...
/* The separator between the name of an array component and the index of one of
   its elements. Indices are 1 based, as they are in Modelica, so an array of
   three components named blk becomes blk_1, blk_2 and blk_3, never blk_0. */
const indexSeparator = '_'

/* A component whose type is one of these is a value, not a node of the
   connection graph, and is never expanded. */
const primitiveTypes = ['Real', 'Integer', 'Boolean', 'String']
...
/**
 * The name one element of an array component is given.
 *
 ...
 *
 * Array *parameters* are deliberately left alone. They are values, not nodes of
 * the connection graph: nothing connects to one, and the elements of one have
 * already been distributed to the components that use them by this point.
 *
...
function expandArrays (allObjects, className = 'the class') {
  ...
  Object.entries(allObjects.instances).forEach(([name, instance]) => {
    if (!(instance.type === 'element' &&
    !['parameter', 'constant'].some(prefix => (instance.type_prefix ?? '').includes(prefix)) &&
    !primitiveTypes.includes(instance.type_specifier))) {
      return
    }
