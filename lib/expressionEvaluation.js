'use strict'
const arrayExpansion = require('../lib/arrayExpansion.js')
...
/* Modelica functions math.js does not know, supplied to it as a scope so that
   they resolve in an expression it evaluates. */
const mathJSScope = {
  fill: (value, size) => new Array(size).fill(value)
}

const variableRegExp = /^\.?(\D\w*)(\.\w+)*/
const primitiveTypes = ['Boolean', 'Integer', 'Real', 'String']

// reg exp that captures the pattern for array comprehensions like {expr for i in range, j in range}
const forLoopRegExp = /^\{\s*([\s\S]+?)\s+for\s+([\s\S]+?)\s*\}$/
const iteratorRegExp = /^\s*([A-Za-z_]\w*)\s+in\s+([\s\S]+)$/
...
/*
 * @param {Object} factor - a logical_factor node, holding `arithmetic_expressions`
 *     and optionally `not` and `relation_operator`
 * @returns {Object} - the factor with every operand as `{ name }`, or the factor
 *     unchanged when it holds nothing that can be normalized
 */
function normalizeLogicalFactor (factor) {
  const names = factor.arithmetic_expressions.map(
    expression => typeof expression === 'string' ? expression : expression?.name)

  // an empty list, or an operand that is a structured node carrying no name
  if (names.length === 0 || names.some(name => name === undefined)) {
    return factor
  }

  /* Logical NOT in JS requires parentheses to be added around the negated
     expression. Otherwise: ! 2 < 3 is interpreted as ! 2 (= false → 0) < 3
     which evaluates to true instead of false. Opening on the first operand and
     closing on the last encloses a factor of any length, so a negated relation
     is parenthesized as a whole rather than around its left side alone. */
  if (factor.not) {
    names[0] = '(' + names[0]
    names[names.length - 1] = names[names.length - 1] + ')'
  }

  return { ...factor, arithmetic_expressions: names.map(name => ({ name })) }
}
...
function stringifyExpression (obj) {
  ...
  if (obj.if_expression ||
    ...
    /* Every logical factor the expression holds is normalized before it is
       rendered, whatever operator it carries. A factor only ever occurs inside a
       logical or an if expression, so this branch covers all of them. */
    return expressionToString.parse(JSON.parse(JSON.stringify(obj, // We use the recursive algorithm of JSON.stringify
      (_, val) => Array.isArray(val?.arithmetic_expressions)
        ? normalizeLogicalFactor(val)
        : val)))
  }
...
/**
 * returns the class definitions as an array.
 *
 * @param {Object} classObject - the class object the defintion originates from
 * @returns {Array<Object>} - the class definitions, empty if there is none.
 */
function getClassDefinitions (classObject) {
  const classDefinitions = classObject?.class_definition ?? classObject?.stored_class_definitions
  if (classDefinitions == null) { return [] }
  return Array.isArray(classDefinitions) ? classDefinitions : [classDefinitions]
}
...
function getClassDefinition (fullClassName, jsons, classObject) {
  ...
  if (!/\./.test(fullClassName)) { // Test for short class definition: to be looked up in the class object
    const classDefinition = jsonQuery.findNestedObjects(classObject, 'class_definition')
      // enables lookup of class definitions within both class_definition and stored_class_definitions
      .concat(jsonQuery.findNestedObjects(classObject, 'stored_class_definitions'))
      .flat() // Either key may hold an array of class definitions
  ...
  for (const classObject of jsonQuery.findNestedObjects(jsons, 'within', within)) {
    for (const classDefinition of getClassDefinitions(classObject)) {
  ...
  for (const p of utilM2j.getMODELICAPATH().map(
    ...
    if (moFilePathNorm.includes(p)) {
      return path.relative(p, moFilePath)
        .replace(/\.mo$/, '')
        // added support for Windows as path.sep is a backslash on Windows
        .split(path.sep).join('.')
    }
  }
}
...
function lookupClassName (typeSpecifier, within, moFilePath) {
  ...
  if (path == null) {
    ...
    if (className == null) {
      /* The class was found, but its file sits under none of the MODELICAPATH
         roots, so its path yields no class name. A

        Usually called by the sibling of the class its actually referring to
      */
      logger.debug(`class name of ${typeSpecifier} used in ${moFilePath} taken from the within clause`)
      return (within == null || within === '' ? typeSpecifier : `${within}.${typeSpecifier}`)
    }
 ...
 * @param {boolean} [withDefaults=false] Whether a declaration with no binding of
 *     its own takes the default of its declared attributes or of its data type.
 ...
 * @property {Array<string>} unresolved - The names of the primitive parameters
 *     that nothing gave a value to, empty unless withDefaults is set. See
 *     `handleSimpleAssignment`.
 *
 * @todo Store in memory parameters and doc of types already processed to avoid redundant lookups.
 */
/**
 * Defaulting of component bindings if the no value is declared
 *
 * A declaration that constrains the parameter - parameter Real k(min=0.1) -
 * names a value the parameter is allowed to take, and the smallest allowed one
 * is the default: the declaration itself says what the parameter may hold, so
 * the value is still one the source wrote down.
 *
 * A declaration that constrains nothing - parameter Real k - gets no default.
 *
 * @param {Object} component - One component, as getComponents returns it
 * @returns {string|undefined} - The default binding, or undefined when the
 *     declaration constrains nothing
 */
function defaultAssignment (component) {
  for (const attribute of ['min', 'max']) {
    const value = component.bindings?.[attribute]
    if (['number', 'boolean'].includes(typeof value) || (typeof value === 'string' && value !== '')) {
      return `${value}`
    }
  }
  return undefined
...
function getParametersAndBindings (classObject, jsons, fetchDoc = false, fetchVariables = false, _instance, _bindings, withDefaults = false) {
  const toReturn = { parameters: [], documentation: [], variables: {}, unresolved: [] }
  ...
  function handleSimpleAssignment (toReturn, component) {
    ...
    const value = (_bindings != null && _bindings[component.identifier])
      ? _bindings[component.identifier] // Bindings passed as arguments override local assignments
      : prependExpression(component.assignment ?? (withDefaults ? defaultAssignment(component) : null))

    if (withDefaults && value == null &&
      primitiveTypes.includes(component.typeSpecifier) &&
      component.typePrefix?.includes('parameter')) {
      toReturn.unresolved.push(identifier)
    }
    toReturn.parameters.push({ name: identifier, value, unit, displayUnit })
  }
  ...
  function handleCompositeComponent (toReturn, component, jsons, classDefinition, fetchDoc, fetchVariables) {
    ...
    toReturn.unresolved.push(...componentData.unresolved)
...
function evalInContext (str, context, prefix) {
  ...
  if (context != null && Object.keys(context).includes(strInContext)) {
    const binding = context[strInContext]
    /* The binding found in the context may itself be an array constructed by
       iterating, which is structural and does not evaluate as an expression. */
    if (typeof binding === 'string' && forLoopRegExp.test(binding.trim())) {
      const values = evalForLoop(binding, context)
      if (values !== null) return values
    }
    return evalInContext(binding, context) // The prefix is not passed to the next call
  }
  ...
  let val = str
  try {
    val = Function('math', `"use strict"; return ${str}`)(math) /* eslint no-new-func: "off" */
  } catch (error) { val = str }

  // includes the case where element-wise operations on arrays are attempted
  // only math.js - evalArrayExpression can be used

  if (typeof str === 'string' && str.includes('[') &&
    (val === str || (typeof val === 'number' && Number.isNaN(val)))) {
    // attempt to evaluate the array expression using math.js
    const arrayValue = evalArrayExpression(str)
...
/**
 * evaluates an expression holding arrays with math.js
 *
 * @param {string} str - a JS array expression
 * @returns {*} - result in the form of an array or undefined
 */
function evalArrayExpression (str) {
  try {
    /* math.evaluate resolves these names itself, so the math. prefix
       stringifyExpression adds for the JS evaluation is not wanted here. */
    const value = math.evaluate(str.replaceAll('math.', ''), { ...mathJSScope })
    return math.isMatrix(value) ? value.toArray() : value
  } catch (error) {
    return undefined
  }
}
...
function evalExpression (expression, context, prefix, _recursive = false) {
  ...
  /* sense the for loop pattern and evaluate it if present */
  if (forLoopRegExp.test(expression.trim())) {
    const values = evalForLoop(expression, context, prefix)
    if (values !== null) {
      return values
    }
  }
...
/**
 * evaluates the range that a for loop takes
 *
 * @param {string} range - the range in terms of "lo:hi" or an expression
 * @param {object} context - the context obejct that has variables and their values
 * @param {string} [prefix] - the rest of the array to append to the end of the array
 * @returns {Array|null} - the value of the arrays or null when it cannot be evaluated
 */
function evalRange (range, context, prefix) {
  const bounds = arrayExpansion.splitModArrayString(range, ':')
  if (bounds.length === 2) {
    const first = evalExpression(bounds[0], context, prefix)
    const last = evalExpression(bounds[1], context, prefix)
  ...
  const values = evalExpression(range, context, prefix)
  return Array.isArray(values) ? values : null
}

/**
 * evaluates an array constructed by iterating: {expr for i in 1:nZon}.
 *
 * the expression is evaluated once per value of the iterator, in a context the
 * iterator has been added to. Several iterators nest, the last one varying
 * fastest, which is the order Modelica builds the array in
 *
 * @param {string} expression - the expression to evaluate
 * @param {object} context - the context object containing variables and their values
 * @param {string} [prefix] - A prefix to prepend to the parameters of the expression
 * @returns {Array|null} - the array, or null when it does not evaluate
 */
function evalForLoop (expression, context, prefix) {
  const match = forLoopRegExp.exec(expression.trim())
  if (match === null) {
    return null
  }
  const body = match[1]
  ...
  const iterators = []
  for (const declaration of arrayExpansion.splitModArrayString(match[2], ',')) {
    const iterator = iteratorRegExp.exec(declaration)
    if (iterator === null) {
      return null
    }
    const values = evalRange(iterator[2], context, prefix)
    if (values === null) {
      return null
    }
    iterators.push({ name: iterator[1], values })
  }

  // recursively expand the iterators to build the array
  let resolved = true
  const expand = (level, bound) => iterators[level].values.map(value => {
    const bindings = { ...bound, [iterators[level].name]: value }
    if (level + 1 < iterators.length) {
      return expand(level + 1, bindings)
    }
    const element = evalExpression(body, { ...context, ...bindings }, prefix)
    if (!['boolean', 'number'].includes(typeof element) && !Array.isArray(element)) {
      resolved = false
    }
    return element
  })

  const values = expand(0, {})
  if (!resolved) {
    logger.debug(`Could not evaluate every element of ${expression}.`)
    return null
  }
  return values
}
...
module.exports.stringifyExpression = stringifyExpression
