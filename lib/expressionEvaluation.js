...
function stringifyExpression (obj) {
  ...
  if (obj.if_expression ||
    ...
    return expressionToString.parse(JSON.parse(JSON.stringify(obj, // We use the recursive algorithm of JSON.stringify
      (_, val) => {
        if (val?.not) {
          // extended to include a list of arithmetic expressions within the NOT operation
          const names = (val.arithmetic_expressions ?? []).map(
            expression => typeof expression === 'string' ? expression : expression?.name)
          // if names has valid elements
          if (names.length > 0 && names.every(name => name !== undefined)) {
            names[0] = '(' + names[0]
            names[names.length - 1] = names[names.length - 1] + ')'
            return { ...val, arithmetic_expressions: names.map(name => ({ name })) }
          }
        }
        return val
      })))
