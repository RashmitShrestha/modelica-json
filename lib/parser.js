...
function getJsons (moFiles, outputFormat, directory, prettyPrint, generateElementary = false, generateCxfCore = false, mode = 'cdl', valueProp = null) {
  ...
  if (outputFormat === 'raw-json') {
    ...
    // Phase 2: every reachable file has now been read, so the expressions of a
    // class can be evaluated against the classes it instantiates, and a class
    // that no other class instantiates can be recognised as a sequence.
