 ...
 */
function getJsons (moFiles, outputFormat, directory, prettyPrint, generateElementary = false, generateCxfCore = false, mode = 'cdl', valueProp = null) {
  ...
  if (outputFormat === 'raw-json') {
    ...
    moFiles.forEach((moFile) => {
      if ((outputFormat === 'cxf' && generateCxfCore && !moFile.split(path.sep).includes('Validation')) ||
        ...
        const simJson = getSimpleJson(moFile, tempJsonDir, parsedFiles, outDir, prettyPrint, outputFormat, generateElementary, generateCxfCore, mode, parsedClasses)
        if (simJson !== null) {
          (
            jsons.push(...simJson)
          )
    ...
    generateObjectsAndCxf(parsedClasses, tempJsonDir, prettyPrint, outputFormat, generateElementary, generateCxfCore, mode, valueProp)
 ...
 * @param {string} mode - Indicates whether to parse a 'cdl' (default) or 'modelica' file with outputFormat='cxf'.
 * @param {Map<string, Object>} [parsedClasses] - collects what the cxf and object layer needs for each parsed file
 * @returns {Array<Object>} - An array of JSON objects representing the Modelica file and all used classes.
 */
function getSimpleJson (moFile, tempDir, parsedFiles, outDir, prettyPrint, outputFormat, generateElementary, generateCxfCore, mode, parsedClasses = new Map()) {
  ...
  if (newInstantiatedClasses !== undefined && newInstantiatedClasses !== null && newInstantiatedClasses.length > 0) {
    newInstantiatedClasses.forEach((element) => {
      const simJson = getSimpleJson(element, tempDir, parsedFiles, outDir, prettyPrint, outputFormat, generateElementary, generateCxfCore, mode, parsedClasses)
      if (simJson !== null) {
        jsons.push(...simJson)
      }
    })
  ...
  if ((outputFormat === 'semantic' || outputFormat === 'cxf') &&
    !parsedClasses.has(path.resolve(moFile))) {
 ...
 */
function generateAllObjectsJson (jsonOutput, moFile, tempDir, prettyPrint, mode, jsons, userValues = null, isSequence = false) {
  ...
  /* Extracted before the file itself is written: extracting a sub-sequence
     rewrites the component that instantiates it so that it names the
     instantiation. */
  const subSequences = isSequence
    ? oe.extractAllSubSequences(allObjectsJson, jsonOutput, jsons, userValues, isSequence)
    : {}
 ...
 * one Modelica file, fouind in value_prop.json by default
 *
...
/**
 * extracts the objects of every class the call parsed.
 *
 * this runs once every file has been read, because a class that another class of
 * the same call instantiates is not a sequence of its own
 *
 ...
 * @returns {void}
 */
function generateObjectsAndCxf (parsedClasses, tempDir, prettyPrint, outputFormat, generateElementary, generateCxfCore, mode, valueProp) {
  ...
  parsedClasses.forEach(({ moFile, jsonOutput, jsons }, resolvedPath) => {
    ...
    const { allObjectsJson, subSequences } = generateAllObjectsJson(
      ...
      isSequence ? readUserValues(valueProp, moFile) : null, isSequence)
...
function generateCxf (jsonOutput, moFile, tempDir, prettyPrint, generateElementary, generateCxfCore, mode, subSequences = {}) {
  ...
  const subBlocks = Object.entries(subSequences).map(
    ([name, objects]) => ({
      blockName: name,
